...
/*       Write a method called isPalindrome with one int parameter called number.
        ...
        to the original number. For example: 121, 12321, 1001 etc.
        ...
        Tip: Be careful with reversing a number, you will need a parameter
        for comparing a reversed number with the starting number (parameter).
...
public class NumberPalindrome {
    public static boolean isPalindrome(int number){
        int reverse=0, lastDigit,tempnumber=Math.abs(number);
        if (number==0)
            return true;
        while(tempnumber>0){
