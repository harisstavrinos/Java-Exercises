...
public class NumberToWords {
    public static void numberToWords(int number){
        ...
        else
        {
            ...
            int nDigitsNumber=getDigitCount(number);
            int nDigitsReverse=getDigitCount(rev);
            int iDigit,temp=rev;


            for(int i=0;i<nDigitsReverse;i++) {
                iDigit=temp%10;
                switch(iDigit){
                    case 0:
            ...
            for(int i=0; i<(nDigitsNumber-nDigitsReverse);i++)
                System.out.println("Zero");
    ...
    public static int reverse(int number){
        int iDigit,rev=0,temp=Math.abs(number),sum=0;
        do{
            iDigit=temp%10;
            sum=sum*10 + iDigit;
            iDigit++;
